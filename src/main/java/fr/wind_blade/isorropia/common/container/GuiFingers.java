...
public class GuiFingers extends GuiContainer {
    ...
    public GuiFingers(InventoryPlayer par1InventoryPlayer) {
        ...
        tex = new ResourceLocation("isorropia", "textures/gui/guifingers.png");
    ...
    protected void drawGuiContainerBackgroundLayer(float par1, int par2, int par3) {
        ...
        if (crystals != null) {
            ...
            for (Aspect a : crystals.getAspects()) {
                ...
                GL11.glPushMatrix();
                GL11.glTranslatef(var5 + ContainerFingers.xx[id] + 7.5f, var6 + ContainerFingers.yy[id] + 8.0f, 0.0f);
        ...
        GL11.glScalef(0.5f, 0.5f, 0.0f);
        int vis = (int) AuraHandler.getVis(tileEntity.player.world, tileEntity.player.getPosition());
        String text = vis + " " + I18n.translateToLocal("workbench.available");
        int ll = fontRenderer.getStringWidth(text) / 2;
        fontRenderer.drawString(text, -ll, 0, (vis < cost) ? 15625838 : 7237358);
        GL11.glScalef(1.0f, 1.0f, 1.0f);
        GL11.glPopMatrix();
        if (cost > 0) {
            if (vis < cost) {
