        ...
        extends Lens
{
    ...
    public void handleTicks(World worldIn, EntityPlayer playerIn, boolean doubleLens) {
        if (!worldIn.isRemote) {
            Entity entity = EntityUtils.getPointedEntity(worldIn, (Entity)playerIn, 0.0D, 1.5D, 5.0F, true);
    ...
    public void handleRenderGameOverlay(World worldIn, EntityPlayer playerIn, ScaledResolution resolution, boolean doubleLens, float partialTicks) {
        Entity entity = EntityUtils.getPointedEntity(worldIn, (Entity)playerIn, 1.0D, 5.0D, 5.0F, true);
        if (entity instanceof EntityLiving) {
            EntityLiving living = (EntityLiving)entity;
            LivingCapability cap = living.getCapability(Common.LIVING_CAPABILITY, null);
    ...
    private void renderEnvyHUD(ScaledResolution scaledResolution, int entityId) {
        ...
        if (entity instanceof EntityLiving) {
            ...
            double pourcentage = ((entity.getCapability(Common.LIVING_CAPABILITY, null).envy * 100) / ((EntityLiving)entity).getHealth() * 20.0F);
            if (pourcentage <= 100.0D) {
                double p = pourcentage * 8.0D / 100.0D;
                double u = 8.0D - p;
                double i = 1.0D - pourcentage / 100.0D;
                Color color = new Color(IsorropiaAPI.ENVY.getColor());
                GL11.glPushMatrix();
                ...
                buffer.pos(x, y + u, 0.0D).tex(0.0D, i).color(color.getRed() / 255.0F, color.getGreen() / 255.0F, color
                        .getBlue() / 255.0F, color.getAlpha() / 255.0F).endVertex();
                buffer.pos(x, y + 8.0D, 0.0D).tex(0.0D, 1.0D).color(color.getRed() / 255.0F, color.getGreen() / 255.0F, color
                        .getBlue() / 255.0F, color.getAlpha() / 255.0F).endVertex();
                buffer.pos(x + 16.0D, y + 8.0D, 0.0D).tex(1.0D, 1.0D).color(color.getRed() / 255.0F, color.getGreen() / 255.0F, color
                        .getBlue() / 255.0F, color.getAlpha() / 255.0F).endVertex();
                buffer.pos(x + 16.0D, y + u, 0.0D).tex(1.0D, i).color(color.getRed() / 255.0F, color.getGreen() / 255.0F, color
                        .getBlue() / 255.0F, color.getAlpha() / 255.0F).endVertex();
                tessellator.draw();
