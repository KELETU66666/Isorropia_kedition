...
public class LensChangeMessage implements IMessage {
    ...
    public void fromBytes(ByteBuf buf) {
        this.lens = IsorropiaAPI.lensRegistry.getValue(buf.readInt());
    ...
    public static class LensChangeMessageHandler implements IMessageHandler<LensChangeMessage, IMessage> {
        public IMessage onMessage(LensChangeMessage message, MessageContext ctx) {
            (ctx.getServerHandler()).player.getServerWorld().addScheduledTask(() -> {
                ...
                LensManager.changeLens(player, message.lens, message.type);
            });
            return null;
        }
    }
