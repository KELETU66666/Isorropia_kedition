...
public class ContainerFingers extends Container
{
    ...
    public ContainerFingers(InventoryPlayer par1InventoryPlayer) {
        ...
        lastVis = -1;
        lastCheck = 0L;
        tileEntity = new TileFinger(par1InventoryPlayer.player);
        tileEntity.inventoryCraft.eventHandler = this;
        ip = par1InventoryPlayer;
        player = par1InventoryPlayer.player;
        tileEntity.getAura();
        addSlotToContainer(new SlotMorphicFingers(tileEntity, par1InventoryPlayer.player, tileEntity.inventoryCraft, craftResult, 15, 160, 64));
        ...
        for (ShardType st : ShardType.values()) {
            if (st.getMetadata() < 6) {
                addSlotToContainer(new SlotCrystal(st.getAspect(), tileEntity.inventoryCraft, st.getMetadata() + 9, ContainerFingers.xx[st.getMetadata()], ContainerFingers.yy[st.getMetadata()]));
            }
        }
        for (int var6 = 0; var6 < 3; ++var6) {
    ...
    public void onCraftMatrixChanged(IInventory par1IInventory) {
        ...
        if (recipe != null) {
            ...
            tileEntity.getAura();
            hasVis = (player.world.isRemote ? (tileEntity.auraVisClient >= vis) : (tileEntity.auraVisServer >= vis));
        ...
        if (hasVis && hasCrystals) {
            slotChangedCraftingGrid(player.world, ip.player, tileEntity.inventoryCraft, craftResult);
        }
        super.detectAndSendChanges();
    }
    ...
    public void onContainerClosed(final EntityPlayer par1EntityPlayer) {
        ...
        if (!this.player.world.isRemote) {
            this.tileEntity.inventoryCraft.eventHandler = null;
        }
        if (!this.ip.player.world.isRemote) {
            for (int i = 0; i < 15; ++i) {
                final ItemStack itemstack = this.tileEntity.getStackInSlot(i);
                if (itemstack != null) {
                    this.ip.player.dropItem(itemstack, false);
                }
            }
        }
    }
    ...
    static {
        ContainerFingers.xx = new int[] { 64, 17, 112, 17, 112, 64 };
        ContainerFingers.yy = new int[] { 13, 35, 35, 93, 93, 115 };
    }
}